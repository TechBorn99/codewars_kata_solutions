...
character cycles through P->Q->R->S->7). A character is "locked in" once the user presses a different key or pauses for
a short period of time (thus, no extra button presses are required beyond what is needed for each letter individually).
The zero key handles spaces, with one press of the key producing a space and two presses producing a zero.

In order to send the message "WHERE DO U WANT 2 MEET L8R" a teen would have to actually do 47 button presses. No wonder
they abbreviated.
...
def presses(phrase):
    ...
    for i in phrase:
        if i.upper() in value1:
            result += 1
        elif i.upper() in value2:
            result += 2
        elif i.upper() in value3:
            result += 3
        elif i.upper() in value4:
            result += 4
        else:
            result += 5
    return result
