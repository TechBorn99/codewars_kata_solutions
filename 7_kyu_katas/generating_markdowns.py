...
Your friend has recently started using Codewars to learn more advanced coding. They have just created their first kata,
and they want to write a proper description for it, using codeblocks, images and hyperlinks.

However, they are struggling to understand how to use Markdown formatting properly, so they decide to ask for your help,
by having you write a program that will generate some of the syntaxes for you.
...
def generate_markdowns(markdown, text, url_or_language):
    ...
    elif 'code' in markdown:
        return '```' + url_or_language + '\n' + text + '\n```'
    else:
        return '[' + text + '](' + url_or_language + ')'


print(generate_markdowns('link', 'hyperlink', 'https://en.wikipedia.org/wiki/Hyperlink'))
